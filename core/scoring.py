...
def generate_nfl_projections(matchup_df, current_week):
    ...
    # FIX: Position-specific TD rates per target (more realistic calibration)
    # WRs: ~2.5% of targets result in TD
    # TEs: ~3.0% of targets result in TD  
    # RBs: ~1.5% of targets result in TD (most RB TDs come from carries, not targets)
    POSITION_TD_RATES = {
        'WR': 0.025,
        'TE': 0.030,
        'RB': 0.015,
    ...
    # We'll use 35 pass attempts as the team average
    ...
    df['matchup_multiplier'] = df.apply(
        lambda row: apply_defensive_matchup(
            1.0, 
            row.get('opp_pass_epa_allowed', 0), 
            row.get('opp_pressure_rate', 0.25), 
            row.get('position', 'WR')
        ), axis=1
    )
    
    # Step 3: Generate Base Projections
    # Use position-specific TD rates
