# core/scoring.py
# LaunchCast NFL — Scoring Engine V6.1
# FIX: Standard Bayesian shrinkage (removed double-shrink),
# separate TD rate shrink with stronger prior
...
# ============================================================================
# BAYESIAN SHRINKAGE (Standard form — no double-shrink)
# ============================================================================
def calculate_shrunk_rate(actual_rate, volume, current_week, league_avg_rate, position='WR'):
    ...
    The prior tightens as the season accumulates real sample.
    No secondary weight_actual multiplier — the prior IS the lever.
    """
    if current_week <= 3:
        prior_volume = 60
...
def shrink_td_rate(actual, targets, league_avg=LEAGUE_AVG_TD_PER_TGT, prior=90):
    """
    TD-per-target shrinkage with a stronger prior.
    TD rate stabilizes far more slowly than target share,
    so we use ~90 targets as the half-trust point.
    """
    if targets <= 0:
        return league_avg
...
def calc_boom_score(row):
    ...
    Normalize components 0-1 first, then weight them.
    """
    vol = min(1.0, row.get('target_share', 0) / 0.30)
    eff = min(1.0, row.get('yds_per_tgt', 11.0) / 15.0)
    rz  = min(1.0, row.get('td_per_tgt', 0.05) / 0.10)
...
def calc_ctx_lift(row, season_avg_td_per_tgt):
    ...
    baseline_expected_tds = season_avg_td_per_tgt * TEAM_AVG_PASS_ATTEMPTS * row.get('target_share', 0.11)
...
def generate_nfl_projections(matchup_df, current_week):
    ...
    # Step 1: Shrunk target share (standard Bayesian, no double-shrink)
    def get_volume(row):
    ...
    # Step 2: Shrunk TD rate (separate function, stronger prior of 90 targets)
    ...
    df['ctx_lift_pp'] = df.apply(
        lambda row: calc_ctx_lift(row, row.get('shrunk_td_per_tgt', LEAGUE_AVG_TD_PER_TGT)),
        axis=1
    )
    ...
    desired_cols = [
        ...
        'target_share', 'shrunk_target_share', 'yds_per_tgt',
